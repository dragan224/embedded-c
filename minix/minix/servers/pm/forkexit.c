 ...
 * process exits first, it continues to occupy a slot until the parent does 
 * WAIT4.
 *
...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  */
  ...
  procs_in_use++;
  if (_flag_ps && !(rmp->mp_flags & PRIV_PROC)) {
     _cnt_ps++;
  }}
 ...
 * become a zombie.
 */
  (!(rmp->mp_flags & PRIV_PROC) && (_flag_um)) { // Oduzimanje memorije _benchmark
    struct vm_usage_info vui;
    memset(&vui, 0, sizeof(vui));
    (void)vm_info_usage(rmp->mp_endpoint, &vui);
    _cnt_um -= vui.vui_total;
  }
...
{
  if (!(rmp->mp_flags & PRIV_PROC) && (_flag_pe)) { // Ako je korisnicki process
    _cnt_pe++;
  }
